...
(function() {
  var config, cssc, cssdata, fs, jsoc, jsoptimze, less, lessToCss, libsUrl, parser, requirejs;
  ...
  cssdata = "@import \"normalize_legacy\";\n@import \"F\";\n@import \"custom\";";

  lessToCss = function() {
    return parser.parse(cssdata, function(e, tree) {
      var css, fd;
      if (e) {
        ...
        try {
          ...
          return fs.closeSync(cssc.output);
        } catch (error) {
  ...
  jsoptimze = function() {
    return requirejs.optimize(jsoc);
  };
  ...
  if (config.js.dir !== config.css.dir) {
    fs.watch(config.js.dir, function(event, filename) {
      console.log("event is: " + event);
      if (filename) {
        console.log("filename provided: " + filename);
        if (/\.js$/.test(filename)) {
          console.log(".js");
          if (!/^o\.js$/.test(filename)) {
    ...
    fs.watch(config.css.dir, function(event, filename) {
      console.log("event is: " + event);
      if (filename) {
        console.log("filename provided: " + filename);
